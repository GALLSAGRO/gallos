...
// Trabaja sobre event_match_id (nuevo sistema)
// -------------------------------------------------------
async function processMatchingBet({ roomId, eventMatchId, eventId, userId, gallo, puntos }) {
  ...
  try {
    ...
    const matchQ = await client.query(
      `SELECT id, estado FROM event_matches
    ...
    if (matchQ.rows[0].estado === 'lista') {
      await client.query(
        `UPDATE event_matches SET estado = 'apostando', betting_opened_at = NOW() WHERE id = $1`,
        [eventMatchId]
      );
    }
    ...
    const betIns = await client.query(
      `INSERT INTO apuestas (user_id, room_id, event_id, event_match_id, gallo, puntos_total, puntos_matched, estado)
    ...
    for (const ob of oppBets.rows) {
      ...
      await client.query(
        `INSERT INTO matches (room_id, event_id, event_match_id, apuesta_a_id, apuesta_b_id,
                              user_a_id, user_b_id, puntos, comision_pct)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 10.00)`,
        [roomId, eventId, eventMatchId,
    ...
    const updatedBet = await client.query(`SELECT * FROM apuestas WHERE id = $1`, [bet.id]);
...
// POST /api/bets
// Colocar apuesta
// Body: { room_id, event_id, event_match_id, gallo: 'R'|'V', puntos }
// -------------------------------------------------------
...
// Pool de apuestas de una pelea (público para la sala)
// -------------------------------------------------------
